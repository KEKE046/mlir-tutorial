...
<!-- vscode-markdown-toc -->
* 1. [MLIR 简介](#MLIR)
	* 1.1. [MLIR 编译管线](#MLIR-1)
	* 1.2. [常见的 Dialect](#Dialect)
	* 1.3. [insight：“及时做优化”](#insight)
	* 1.4. [MLIR 的用处](#MLIR-1)
	* 1.5. [MLIR 的缺点](#MLIR-1)
* 2. [MLIR 基本用法](#MLIR-1)
	* 2.1. [IR 基本结构](#IR)
	* 2.2. [MLIR 基本工程模板](#MLIR-1)
		* 2.2.1. [配置 clangd](#clangd)
	* 2.3. [MLIR 的读入、输出](#MLIR-1)
	* 2.4. [用代码生成 MLIR](#MLIR-1)
* 3. [MLIR Op 的结构](#MLIROp)
	* 3.1. [Attribute 和 Operand](#AttributeOperand)
	* 3.2. [Attribute, Value 和 Type](#AttributeValueType)
* 4. [MLIR 的类型转换](#MLIR-1)
	* 4.1. [Op 的类型转换](#Op)
	* 4.2. [Type / Attribute 的类型转换](#TypeAttribute)
* 5. [MLIR 的图结构](#MLIR-1)
	* 5.1. [MLIR 数据流图结构](#MLIR-1)
	* 5.2. [MLIR 数据流图的遍历与修改](#MLIR-1)
	* 5.3. [MLIR 控制流图的遍历与修改](#MLIR-1)
* 6. [基本的 Dialect 工程](#Dialect-1)
	* 6.1. [TableGen 工程模板](#TableGen)
	* 6.2. [Tablegen Language Server](#TablegenLanguageServer)
	* 6.3. [IR 的默认定义与实现](#IR-1)
		* 6.3.1. [TableGen 文件](#TableGen-1)
		* 6.3.2. [头文件](#)
		* 6.3.3. [库文件](#-1)
		* 6.3.4. [程序入口](#-1)
* 7. [TableGen Op 定义详解](#TableGenOp)
	* 7.1. [Attribute、Type、Constraint](#AttributeTypeConstraint)
		* 7.1.1. [内置 Attribute](#Attribute)
		* 7.1.2. [内置的 Type](#Type)
		* 7.1.3. [为什么 Attribute 和 Type 都是 Constraint](#AttributeTypeConstraint-1)
	* 7.2. [Verifier：发现IR错误](#VerifierIR)
		* 7.2.1. [emitError](#emitError)
		* 7.2.2. [LogicalResult](#LogicalResult)
	* 7.3. [Variadic：可变参数](#Variadic)
		* 7.3.1. [多个可变参数：AttrSizedOperandSegments](#AttrSizedOperandSegments)
	* 7.4. [AssemblyFormat：更易读的输出](#AssemblyFormat)
		* 7.4.1. [常用关键字](#-1)
		* 7.4.2. [额外 attr 字典](#attr)
		* 7.4.3. [输出 type](#type)
		* 7.4.4. [可选输出：Optional、UnitAttr](#OptionalUnitAttr)
	* 7.5. [Builder：自定义 create 函数](#Buildercreate)
		* 7.5.1. [默认Builder](#Builder)
		* 7.5.2. [自定义builder](#builder)
	* 7.6. [自定义函数](#-1)
		* 7.6.1. [header target](#headertarget)
	* 7.7. [使用 Trait](#Trait)
		* 7.7.1. [内存副作用：SideEffectInterfaces](#SideEffectInterfaces)
		* 7.7.2. [类型推断：InferTypeOpInterface](#InferTypeOpInterface)
	* 7.8. [函数：FunctionOpTrait](#FunctionOpTrait)
		* 7.8.1. [定义 Return](#Return)
		* 7.8.2. [定义 Function](#Function)
		* 7.8.3. [定义 Call](#Call)
* 8. [添加 Pass](#Pass)
	* 8.1. [Pass 工程模板](#Pass-1)
	* 8.2. [Pass 定义详解](#Pass-1)
		* 8.2.1. [指定 Pass 在哪个 Op 上运行](#PassOp)
		* 8.2.2. [带参数的 Pass](#Pass-1)
	* 8.3. [简单的 DCE Pass 实现](#DCEPass)
		* 8.3.1. [定义](#-1)
		* 8.3.2. [实现](#-1)
* 9. [Pattern Rewrite](#PatternRewrite)
	* 9.1. [Pattern Rewrite](#PatternRewrite-1)
		* 9.1.1. [描述 Pattern](#Pattern)
		* 9.1.2. [调用 Pattern](#Pattern-1)
		* 9.1.3. [Depedent Dialect & Linking](#DepedentDialectLinking)
	* 9.2. [Dialect Convertion (Type Conversion)](#DialectConvertionTypeConversion)
		* 9.2.1. [TypeConverter](#TypeConverter)
		* 9.2.2. [Conversion Pattern：自动做 Operand 的类型转换](#ConversionPatternOperand)
		* 9.2.3. [类型转换的细节与 Debug](#Debug)
		* 9.2.4. [使用自己的 materialization](#materialization)
	* 9.3. [使用 MLIR 里已有的 Pattern 做多步转换](#MLIRPattern)
* 10. [自定义 Dialect 类型](#Dialect-1)
* 11. [TIPS](#TIPS)
	* 11.1. [如何找头文件、找想要的函数](#-1)
	* 11.2. [如何找需要连接的库](#-1)
	* 11.3. [如何加快编译速度](#-1)
	* 11.4. [去 MLIR 里抄代码](#MLIR-1)
* 12. [MLIR 的批判：C++ v.s. Rust](#MLIRCv.s.Rust)
* 13. [Issue & Reply](#IssueReply)
...
##  1. <a name='MLIR'></a>MLIR 简介

###  1.1. <a name='MLIR-1'></a>MLIR 编译管线
...
###  1.2. <a name='Dialect'></a>常见的 Dialect
...
###  1.3. <a name='insight'></a>insight：“及时做优化”
...
##  2. <a name='MLIR-1'></a>MLIR 基本用法

###  2.1. <a name='IR'></a>IR 基本结构
...
###  2.2. <a name='MLIR-1'></a>MLIR 基本工程模板
...
####  2.2.1. <a name='clangd'></a>配置 clangd
...
###  2.3. <a name='MLIR-1'></a>MLIR 的读入、输出
...
###  2.4. <a name='MLIR-1'></a>用代码生成 MLIR
...
###  3.1. <a name='AttributeOperand'></a>Attribute 和 Operand
...
###  3.2. <a name='AttributeValueType'></a>Attribute, Value 和 Type
...
##  4. <a name='MLIR-1'></a>MLIR 的类型转换

###  4.1. <a name='Op'></a>Op 的类型转换
...
###  4.2. <a name='TypeAttribute'></a>Type / Attribute 的类型转换
...
###  5.1. <a name='MLIR-1'></a>MLIR 数据流图结构
...
###  5.2. <a name='MLIR-1'></a>MLIR 数据流图的遍历与修改
...
###  5.3. <a name='MLIR-1'></a>MLIR 控制流图的遍历与修改
...
##  6. <a name='Dialect-1'></a>基本的 Dialect 工程
...
###  6.1. <a name='TableGen'></a>TableGen 工程模板
...
###  6.2. <a name='TablegenLanguageServer'></a>Tablegen Language Server
...
###  6.3. <a name='IR-1'></a>IR 的默认定义与实现

####  6.3.1. <a name='TableGen-1'></a>TableGen 文件
...
##  7. <a name='TableGenOp'></a>TableGen Op 定义详解

上一节介绍了 MLIR 工程的骨架，现在我们为其添砖加瓦，让 IR 的定义、输入、输出更简单。

###  7.1. <a name='AttributeTypeConstraint'></a>Attribute、Type、Constraint
...
####  7.1.1. <a name='Attribute'></a>内置 Attribute
...
####  7.1.3. <a name='AttributeTypeConstraint-1'></a>为什么 Attribute 和 Type 都是 Constraint
...
###  7.2. <a name='VerifierIR'></a>Verifier：发现IR错误
...
###  7.3. <a name='Variadic'></a>Variadic：可变参数
...
####  7.3.1. <a name='AttrSizedOperandSegments'></a>多个可变参数：AttrSizedOperandSegments
...
###  7.4. <a name='AssemblyFormat'></a>AssemblyFormat：更易读的输出
...
###  7.5. <a name='Buildercreate'></a>Builder：自定义 create 函数

Builder 会在 `builder.create<XXXOp>()` 的时候被调用，一个更简单的 builder 可以让创建 Op 更快捷。

####  7.5.1. <a name='Builder'></a>默认Builder
...
####  7.5.2. <a name='builder'></a>自定义builder
...
####  7.6.1. <a name='headertarget'></a>header target
...
####  7.7.1. <a name='SideEffectInterfaces'></a>内存副作用：SideEffectInterfaces
...
####  7.7.2. <a name='InferTypeOpInterface'></a>类型推断：InferTypeOpInterface
...
###  7.8. <a name='FunctionOpTrait'></a>函数：FunctionOpTrait
...
####  7.8.1. <a name='Return'></a>定义 Return
...
####  7.8.2. <a name='Function'></a>定义 Function
...
###  8.2. <a name='Pass-1'></a>Pass 定义详解

####  8.2.1. <a name='PassOp'></a>指定 Pass 在哪个 Op 上运行
...
####  8.2.2. <a name='Pass-1'></a>带参数的 Pass
...
###  8.3. <a name='DCEPass'></a>简单的 DCE Pass 实现
...
##  9. <a name='PatternRewrite'></a>Pattern Rewrite
...
###  9.1. <a name='PatternRewrite-1'></a>Pattern Rewrite

####  9.1.1. <a name='Pattern'></a>描述 Pattern
...
####  9.1.2. <a name='Pattern-1'></a>调用 Pattern
...
####  9.1.3. <a name='DepedentDialectLinking'></a>Depedent Dialect & Linking
...
###  9.2. <a name='DialectConvertionTypeConversion'></a>Dialect Convertion (Type Conversion)
...
####  9.2.2. <a name='ConversionPatternOperand'></a>Conversion Pattern：自动做 Operand 的类型转换
...
####  9.2.3. <a name='Debug'></a>类型转换的细节与 Debug
...
####  9.2.4. <a name='materialization'></a>使用自己的 materialization
...
###  9.3. <a name='MLIRPattern'></a>使用 MLIR 里已有的 Pattern 做多步转换
...
##  10. <a name='Dialect-1'></a>自定义 Dialect 类型
...
###  11.4. <a name='MLIR-1'></a>去 MLIR 里抄代码
...
##  12. <a name='MLIRCv.s.Rust'></a>MLIR 的批判：C++ v.s. Rust
...
##  13. <a name='IssueReply'></a>Issue & Reply
